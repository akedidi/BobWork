...
export function NumberRow({ title, value, min, step, onChange }: { title: string; value: number; min: number; step?: number; onChange: (value: number) => void }) { return <label className="settings-row"><RowText title={title} /><input className="settings-number" type="number" value={value} min={min} step={step} onChange={event => onChange(Number(event.target.value))} /></label> }
