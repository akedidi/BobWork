...
import { exportPluginZip, comparePluginVersion, getPluginExtensionStatus, getPluginMcpStatus, getPluginResourceStatus, getPluginVersions, rollbackPluginVersion, installPluginUpdate, testPluginMcp, validatePlugin } from '../../lib/ipc';
...
import { metadataOf, isEnabled, isProtectedBuiltin, pluginKindLabel, permissionLabel, capabilityLabel, PluginMetadata } from '../../lib/pluginUtils';
...
export default function PluginDetail({ plugin, mcpRevision, toggling, openCommissioning, onClose, onToggle, onEdit, onDelete, onStatus, onOpenIntegrations, onUseSchedule, onVersionChanged }: {
  ...
  const capabilities = manifest.capabilities?.filter((value: any) => value !== 'prompt').map(capabilityLabel) ?? []
...
function localResourceKindLabel(kind: string) {
  return ({
    oauth: 'OAuth',
    mcp: 'MCP',
        ...
        {rows.map((resource: any) => {
          ...
          return (
            <div className="plugin-mcp-card" key={resource.id}>
              ...
              <p>{resource.message || localResourceKindLabel(resource.kind)}</p>
              {'setupHint' in resource && resource.setupHint && <p className="settings-note">{resource.setupHint}</p>}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 6 }}>
                <small>{localResourceKindLabel(resource.kind)}</small>
                {showConfigure && tab && (
                  <button
                    type="button"
                    className="link-btn"
