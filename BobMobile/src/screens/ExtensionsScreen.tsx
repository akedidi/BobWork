    ...
    <View style={commonStyles.screen}>
      ...
      {loading && !catalog.plugins.length && !catalog.skills.length ? <View style={commonStyles.empty}><ActivityIndicator color={colors.accent} /></View> : (
        <ScrollView style={styles.catalogList} contentContainerStyle={styles.list} keyboardShouldPersistTaps="handled">
          ...
          {section === 'skills' && skills.map(item => <CatalogRow key={`${item.scope}-${item.slug}`} title={item.name} description={item.description} builtin={item.builtin} favorite={item.favorite} enabled={item.enabled} busy={busyId === item.slug} detail={item.nestedCount ? t('nestedCount', { count: item.nestedCount }) : item.scope} onPress={() => setSelected({ kind: 'skill', item })} onFavorite={() => favorite('skill', item.slug, !item.favorite, item.scope)} onToggle={value => toggleSkill(item, value)} />)}
      ...
      <ExtensionDetail
        ...
        onUpdatePlugin={item => void mutate(item.id, () => api!.installPluginUpdate(item.id), 'extensionUpdateFailed')}
        onOpenSkill={item => setSelected({ kind: 'skill', item })}
      />
    </View>
  )
}
...
function ExtensionDetail({ selected, catalogSkills, busyId, onClose, onFavorite, onTogglePlugin, onToggleSkill, onUpdatePlugin, onOpenSkill }: { selected: Selected | null; catalogSkills: CatalogSkill[]; busyId: string | null; onClose: () => void; onFavorite: (kind: Selected['kind'], id: string, value: boolean, scope?: string) => void; onTogglePlugin: (item: CatalogPlugin, enabled: boolean) => void; onToggleSkill: (item: CatalogSkill, enabled: boolean) => void; onUpdatePlugin: (item: CatalogPlugin) => void; onOpenSkill: (item: CatalogSkill) => void }) {
  ...
  const parentSkill = selected.kind === 'skill' && selected.item.parentSlug
    ? catalogSkills.find(item => item.slug === selected.item.parentSlug) ?? null
    : null
  return <AppModal visible title={title} onClose={onClose}><ScrollView contentContainerStyle={styles.detailContent}><View style={styles.detailHeader}>{selected.kind === 'plugin' ? <PluginGlyph plugin={selected.item} size={32} /> : null}<View style={styles.detailTitleWrap}><Text style={styles.detailTitle}>{selected.item.name}</Text><Text style={styles.detailKind}>{selected.kind === 'integration' ? t('integrations') : selected.item.builtin ? t('builtinItem') : t('personalItem')}</Text></View><Pressable onPress={() => onFavorite(selected.kind, id, !favorite, selected.kind === 'skill' ? selected.item.scope : undefined)} accessibilityLabel={favorite ? t('removeFavorite') : t('addFavorite')}><Ionicons name={favorite ? 'star' : 'star-outline'} size={25} color={favorite ? colors.warning : colors.textMuted} /></Pressable></View>{selected.kind !== 'integration' && selected.item.description ? <Text style={styles.detailDescription}>{selected.item.description}</Text> : null}
    ...
    {selected.kind === 'skill' ? <><StateControl enabled={selected.item.enabled} busy={busyId === selected.item.slug} onChange={value => onToggleSkill(selected.item, value)} /><DetailSection title={t('scope')} values={selected.item.scope ? [selected.item.scope] : []} />{parentSkill ? <Pressable onPress={() => onOpenSkill(parentSkill)}><Text style={styles.parentLink}>{t('backToParentSkill')}</Text></Pressable> : null}{(selected.item.childSkills?.length ?? 0) > 0 ? <View style={styles.detailSection}><Text style={styles.detailSectionTitle}>{t('nestedSkills')}</Text><Text style={styles.meta}>{t('nestedSkillsHint')}</Text>{selected.item.childSkills!.map(child => <Pressable key={child.slug} style={styles.nestedSkillRow} onPress={() => onOpenSkill(child)}><View style={{ flex: 1, minWidth: 0 }}><Text style={styles.nestedSkillName}>{child.name}</Text><Text style={styles.nestedSkillDesc} numberOfLines={2}>{child.description || child.slug}</Text></View><Text style={styles.meta}>{child.enabled ? t('enabled') : t('disabled')}</Text></Pressable>)}</View> : null}</> : null}
