...
case "$cmd" in
  ...
  build)
    echo "▶  Building Bob Work from clean Release outputs..."
    bash "$SCRIPT_DIR/release-build.sh" app
    echo "✓  Built: $BUNDLE_DIR/macos/Bob Work.app"
    ;;

  # ── Full build + DMG ───────────────────────────────────────
  dmg)
    echo "▶  Building Bob Work DMG from clean Release outputs..."
    bash "$SCRIPT_DIR/release-build.sh" dmg
