...
export async function clickSidebar(label: string) {
  ...
  const item = label === 'Nouveau chat'
    ...
    : label === 'Réglages'
      ? $('[role="button"][aria-label="Réglages"], [role="button"][aria-label="Settings"], [role="button"][aria-label="Ajustes"]')
      : sidebarItem(label)
...
async function completeOnboardingIfNeeded() {
  ...
  const waitForApplicationShell = () => browser.waitUntil(async () => (
    await setup.isExisting()
    || await $('div.sidebar-nav').isExisting()
  ), {
    timeout: 30_000,
    interval: 250,
    timeoutMsg: "Bob Work n'a affiché ni l'onboarding ni la navigation principale.",
  })
  try {
    await waitForApplicationShell()
  } catch {
    // On a pristine E2E profile the native database migration can complete
    // after WebKit's first document load. One reload exercises the normal
    // application bootstrap against that freshly-created database.
    await browser.refresh()
    await waitForApplicationShell().catch(async (error) => {
      const source = await browser.getPageSource().catch(() => '')
      const url = await browser.getUrl().catch(() => '')
      throw new Error(`${String(error)} URL=${url} DOM=${source.slice(0, 1_000)}`)
    })
  }
...
export async function ensureHomeReady() {
  ...
  await $('div.sidebar-nav').waitForDisplayed({ timeout: 30_000 })
