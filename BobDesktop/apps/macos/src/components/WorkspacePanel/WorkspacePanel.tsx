...
const PANEL_EDGE_GUTTER = 120
const PANEL_KEYBOARD_STEP = 32

function panelWidthBounds(panel: HTMLElement) {
  const containerWidth = panel.parentElement?.getBoundingClientRect().width || window.innerWidth
  return {
    min: Math.min(PANEL_MIN_WIDTH, Math.max(240, containerWidth - PANEL_EDGE_GUTTER)),
    max: Math.max(PANEL_MIN_WIDTH, containerWidth - PANEL_EDGE_GUTTER),
  }
}

function clampPanelWidth(panel: HTMLElement, width: number) {
...
export default function WorkspacePanel({ detail, live, running, request, onClose, variant = 'chat' }: {
  ...
  return <aside
    className={`workspace-panel workspace-panel--${variant}${panelWidth === null ? '' : ' is-resized'}`}
    aria-label="Aperçus et activité"
  ...
  >
    <div
      ...
      onPointerMove={event => {
        ...
        // The panel is attached to the right edge: moving left makes it wider.
        setPanelWidth(clampPanelWidth(panel, drag.startWidth + drag.startX - event.clientX))
      }}
      onPointerUp={event => {
      ...
      onKeyDown={event => {
        ...
        event.preventDefault()
        const bounds = panelWidthBounds(panel)
        const currentWidth = panelWidth ?? panel.getBoundingClientRect().width
        if (event.key === 'Home') setPanelWidth(bounds.min)
        else if (event.key === 'End') setPanelWidth(bounds.max)
        else setPanelWidth(clampPanelWidth(panel, currentWidth + (event.key === 'ArrowLeft' ? PANEL_KEYBOARD_STEP : -PANEL_KEYBOARD_STEP)))
      }}
    />
