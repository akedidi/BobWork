...
import doclingIcon from '../../assets/plugin-icons/docling.svg'
...
import awsIcon from '../../assets/plugin-icons/aws.svg'
import azureIcon from '../../assets/plugin-icons/azure.svg'
import gcpIcon from '../../assets/plugin-icons/gcp.svg'
import ibmCloudIcon from '../../assets/plugin-icons/ibm-cloud.svg'
import watsonxIcon from '../../assets/plugin-icons/watsonx.svg'
import ibmDb2Icon from '../../assets/plugin-icons/ibm-db2.svg'
import ibmZIcon from '../../assets/plugin-icons/ibm-z.svg'
import qiskitIcon from '../../assets/plugin-icons/qiskit.png'
...
import openshiftIcon from '../../assets/plugin-icons/openshift.svg'
...
import terraformIcon from '../../assets/plugin-icons/terraform.svg'
import ansibleIcon from '../../assets/plugin-icons/ansible.svg'
import analyticsIcon from '../../assets/plugin-icons/analytics.svg'
import wordIcon from '../../assets/plugin-icons/word.svg'
import codegraphIcon from '../../assets/plugin-icons/codegraph.svg'
import mapIcon from '../../assets/plugin-icons/map.svg'
...
const ICONS = new Set<string>([
  ...
  'github', 'aws', 'azure', 'gcp', 'ibm-cloud', 'watsonx', 'openshift', 'terraform', 'ansible', 'analytics', 'ibm-db2', 'ibm-z', 'qiskit',
  'slack', 'monday', 'outlook', 'teams', 'calendar', 'onedrive', 'meeting',
  'agentic', 'designer', 'consultant', 'rfp', 'product', 'delivery', 'change', 'architecture', 'docling', 'plugin', 'codegraph', 'map',
])
...
const PLUGIN_ID_ICON: Record<string, PluginIconId> = {
  ...
  'builtin-azure': 'azure',
  'builtin-gcp': 'gcp',
  'builtin-ibm-cloud': 'ibm-cloud',
  'builtin-ibm-watsonx-ai': 'watsonx',
  'builtin-openshift': 'openshift',
  'builtin-terraform': 'terraform',
  'builtin-ansible': 'ansible',
  'builtin-data-analytics': 'analytics',
  'builtin-ibm-db2': 'ibm-db2',
  'builtin-ibm-z': 'ibm-z',
  'builtin-ibm-qiskit': 'qiskit',
  'builtin-codegraph': 'codegraph',
  'builtin-map-tools': 'map',
...
const LOCAL_ICON_RULES: Array<{ keys: string[]; icon: PluginIconId }> = [
  ...
  { keys: ['docling', 'bob-work-docling', 'builtin-docling'], icon: 'docling' },
  ...
  { keys: ['amazon-web-services', 'amazon web services', 'builtin-aws', ' aws '], icon: 'aws' },
  { keys: ['microsoft-azure', 'microsoft azure', 'builtin-azure'], icon: 'azure' },
  { keys: ['google-cloud-platform', 'google cloud', 'builtin-gcp', ' gcp '], icon: 'gcp' },
  { keys: ['ibm-cloud', 'ibm cloud'], icon: 'ibm-cloud' },
  { keys: ['ibm-watsonx', 'ibm watsonx', 'watsonx.ai', 'watsonx.data', 'watsonx.governance'], icon: 'watsonx' },
  { keys: ['red-hat-openshift', 'red hat openshift', 'openshift'], icon: 'openshift' },
  { keys: ['hashicorp-terraform', 'hashicorp terraform', 'terraform'], icon: 'terraform' },
  { keys: ['red-hat-ansible', 'red hat ansible', 'ansible'], icon: 'ansible' },
  { keys: ['data analytics', 'data-analysis', 'analytics'], icon: 'analytics' },
  { keys: ['ibm-db2', 'ibm db2'], icon: 'ibm-db2' },
  { keys: ['ibm-z', 'ibm z', 'zowe'], icon: 'ibm-z' },
  { keys: ['ibm-qiskit', 'ibm qiskit', 'qiskit'], icon: 'qiskit' },
  { keys: ['codegraph', 'code graph', 'semantic-search', 'callers', 'callees'], icon: 'codegraph' },
  { keys: ['map-tools', 'cartes', 'itinéraires', 'itineraires', 'geocoding', 'routing'], icon: 'map' },
...
function isPlaceholderIcon(value: string | undefined): boolean {
  return !value || value === 'plugin' || value === 'agentic'
}
...
export function resolveSkillIcon(skill: {
  ...
  if (declared && !isPlaceholderIcon(declared) && (ICONS.has(declared) || isRemotePluginIcon(declared))) {
    return declared
  }
...
export function resolvePluginIcon(plugin: Pick<Plugin, 'id' | 'manifest'> & { name?: string }): string {
  const manifest = plugin.manifest as { icon?: string; agentic?: boolean; slug?: string; description?: string } | undefined
