...
import Sidebar, { synchronizedSpinnerDelay, USAGE_REFRESH_INTERVAL_MS, clampConversationMenuPosition } from './Sidebar'
...
describe('Sidebar', () => {
  ...
  beforeEach(() => {
    ...
    mocks.getConversation.mockResolvedValue(conversations[0])
  ...
  it('ajoute immédiatement une conversation créée depuis Bob Mobile', async () => {
    const mobileConversation = {
      ...useAppStore.getState().conversations[0],
      id: 'conversation-mobile',
      title: 'Conversation mobile en direct',
      date: '2026-08-30T05:30:00Z',
    }
    mocks.getConversation.mockResolvedValueOnce(mobileConversation)

    render(<MemoryRouter><Sidebar /></MemoryRouter>)

    await waitFor(() => expect(mocks.listeners.has('conversation-updated')).toBe(true))
    // The global refresh may fail transiently; the changed row must still be
    // inserted from the targeted lookup triggered by the desktop event.
    mocks.getConversations.mockRejectedValueOnce(new Error('global refresh delayed'))
    act(() => {
      mocks.listeners.get('conversation-updated')?.({ payload: mobileConversation.id })
    })

    expect(await screen.findByText('Conversation mobile en direct')).toBeVisible()
    expect(useAppStore.getState().conversations[0].id).toBe(mobileConversation.id)
  })
  ...
  it('routes a recent conversation to its thread instead of the home view', async () => {
    render(
    ...
    fireEvent.click(await screen.findByText('Conversation locale'))
  ...
  it('affiche email et instance entre Réglages et Bobcoins quand le profil API est connu', async () => {
    mocks.getUsageStatus.mockResolvedValue({
      ...
      remainingAmount: 90,
      unit: 'Bobcoins',
      accountEmail: 'akedidi@fr.ibm.com',
      instanceName: 'IBM Internal',
      message: 'ok',
    })
    render(
      <MemoryRouter>
    ...
    const account = await screen.findByText('akedidi@fr.ibm.com - IBM Internal')
    expect(account).toBeVisible()
    const settings = screen.getByRole('button', { name: 'Réglages' })
    const meter = screen.getByRole('progressbar')
    expect(meter.compareDocumentPosition(account) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(account.compareDocumentPosition(settings) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })

  it('n’affiche pas la ligne compte sans e-mail de profil', async () => {
    render(
      <MemoryRouter>
        <Sidebar />
      </MemoryRouter>,
  ...
  it('relit le statut Bobcoins chaque minute pour les autres apps Bob', async () => {
    ...
    try {
      ...
      const callsAfterMount = mocks.getUsageStatus.mock.calls.length
      expect(callsAfterMount).toBeGreaterThan(0)

      await act(async () => {
        vi.advanceTimersByTime(USAGE_REFRESH_INTERVAL_MS)
      })
      expect(mocks.getUsageStatus.mock.calls.length).toBeGreaterThan(callsAfterMount)
      expect(mocks.getUsageStatus).toHaveBeenCalledWith(true)
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('clamps the context menu inside the viewport', () => {
    expect(clampConversationMenuPosition(12, 18, 800, 600)).toEqual({ x: 12, y: 18 })
    expect(clampConversationMenuPosition(790, 590, 800, 600)).toEqual({ x: 572, y: 424 })
  })

  it('exécute les actions du menu contextuel malgré le pointerdown global', async () => {
    render(
      <MemoryRouter>
        <Sidebar />
      </MemoryRouter>,
    ...
    fireEvent.contextMenu(await screen.findByText('Conversation locale'), { clientX: 40, clientY: 80 })
    const menu = await screen.findByRole('menu', { name: 'Actions de la conversation' })
    expect(menu).toHaveClass('conversation-context-menu')
    expect(menu).toHaveStyle({ top: '80px', left: '40px' })

    const pin = screen.getByRole('menuitem', { name: /Épingler le chat/i })
    fireEvent.pointerDown(pin)
    fireEvent.click(pin)

    await waitFor(() => {
      expect(mocks.updateConversation).toHaveBeenCalledWith('conversation-1', { pinned: true })
    })
    expect(screen.queryByRole('menu', { name: 'Actions de la conversation' })).not.toBeInTheDocument()
  })

  it('renomme une conversation depuis le menu contextuel', async () => {
    render(
      <MemoryRouter>
        <Sidebar />
      </MemoryRouter>,
    ...
    fireEvent.contextMenu(await screen.findByText('Conversation locale'))
    fireEvent.pointerDown(screen.getByRole('menuitem', { name: 'Renommer le chat' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Renommer le chat' }))

    const input = await screen.findByDisplayValue('Conversation locale')
    fireEvent.change(input, { target: { value: 'Brief client' } })
    fireEvent.blur(input)

    await waitFor(() => {
      expect(mocks.updateConversation).toHaveBeenCalledWith('conversation-1', { title: 'Brief client' })
    })
  })
