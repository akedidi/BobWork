...
export function clampConversationMenuPosition(
  x: number,
  y: number,
  ...
  const maxX = Math.max(CONTEXT_MENU_EDGE, viewportWidth - width - CONTEXT_MENU_EDGE)
  const maxY = Math.max(CONTEXT_MENU_EDGE, viewportHeight - height - CONTEXT_MENU_EDGE)
  return {
    x: Math.min(Math.max(CONTEXT_MENU_EDGE, x), maxX),
    y: Math.min(Math.max(CONTEXT_MENU_EDGE, y), maxY),
  }
}
...
export function reconcileConversationUpdate(
  conversations: Conversation[],
  changed: Conversation | null,
  changedId = changed?.id,
) {
  const remaining = conversations.filter(conversation => conversation.id !== changedId)
  if (!changed || changed.archived) return remaining
  return [changed, ...remaining].sort((left, right) => Date.parse(right.date) - Date.parse(left.date))
}
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    listen<string>('conversation-updated', event => {
      let changedConversation: Conversation | null | undefined

      // Fetch the changed row directly so a conversation created from Bob
      // Mobile appears without depending on a second, global list request.
      getConversation(event.payload)
        .then(changed => {
          changedConversation = changed
          if (!disposed) {
            setConversations(reconcileConversationUpdate(useAppStore.getState().conversations, changed, event.payload))
            setSidebarLoadError(null)
          }
        })
        .catch(error => {
          if (!disposed) setSidebarLoadError(errorMessage(error, 'Impossible d’actualiser la conversation.'))
        })

      getConversations()
        .then(next => {
          if (!disposed) {
            setConversations(changedConversation === undefined
              ? next
              : reconcileConversationUpdate(next, changedConversation, event.payload))
            setSidebarLoadError(null)
          }
        })
        .catch(error => {
  ...
  const openConversationMenu = (conversationId: string, clientX: number, clientY: number) => {
    const position = clampConversationMenuPosition(
      clientX,
      clientY,
      window.innerWidth,
      window.innerHeight,
    )
    contextMenuOpenedAt.current = performance.now()
    setContextMenu({ ...position, conversationId })
  }
  ...
  useEffect(() => {
    const onPointerDown = (event: PointerEvent) => {
      if (performance.now() - contextMenuOpenedAt.current < 280) return
  ...
  return (
    <div className="sidebar" style={{ backdropFilter: 'blur(30px)' }}>
      ...
      {notificationsOpen ? (
        ...
        <>
          ...
          <div className="sidebar-content">
            {/* Pinned */}
            <div className="sidebar-section-label" style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'none', paddingLeft: 8 }}>{t('nav.pinned')}</div>
            {pinnedConversations.length === 0 && pinnedTasks.length === 0 ? <div style={{ padding: '7px 10px 16px', color: 'var(--text-muted)', fontSize: 11.5 }}>{t('nav.pinnedEmpty')}</div> : <>
              {pinnedConversations.map(conversation => <div
                ...
                onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); openConversationMenu(conversation.id, e.clientX, e.clientY) }}
              ...
              >
                ...
                <button className="sidebar-inline-action" onClick={event => { event.stopPropagation(); setConversationPinned(conversation.id, false) }} title={t('nav.unpinConversation')} aria-label={t('nav.unpinNamed', { title: conversation.title })}><PinIcon filled /></button>
              </div>)}
            ...
            {projects.length === 0 ? (
              <div style={{ padding: '8px 12px', fontSize: 13, color: 'var(--text-muted)' }}>{t('nav.noProjects')}</div>
            ) : (
                      ...
                      <div style={{ paddingLeft: 24, marginTop: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
                        {projConvs.map(c => (
                          <div
                            ...
                            onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); openConversationMenu(c.id, e.clientX, e.clientY) }}
            ...
            <div aria-label="Conversations récentes">
              {visibleRecentConversations.length === 0 ? (
                <div style={{ padding: '7px 10px 16px', color: 'var(--text-muted)', fontSize: 11.5 }}>{t('nav.noConversationsList')}</div>
      ...
      <div className="sidebar-footer" style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0, marginBottom: 8 }}>
        ...
        {usage?.accountEmail && (
          <div
            className="sidebar-account-line"
            title={usage.instanceName ? `${usage.accountEmail} - ${usage.instanceName}` : usage.accountEmail}
          >
            {usage.accountEmail}
      ...
      {contextMenu && createPortal(
        ...
        >
          <button
            ...
            onClick={() => {
              const pinned = conversations.find(c => c.id === contextMenu.conversationId)?.pinned
              setConversationPinned(contextMenu.conversationId, !pinned)
              setContextMenu(null)
            }}
          >
            <PinIcon filled={conversations.find(c => c.id === contextMenu.conversationId)?.pinned} />
            {conversations.find(c => c.id === contextMenu.conversationId)?.pinned ? t('nav.unpinChat') : t('nav.pinChat')}
          </button>
          <button
            type="button"
            role="menuitem"
            className="conversation-context-menu__item"
            onClick={() => {
              startEdit(contextMenu.conversationId, conversations.find(c => c.id === contextMenu.conversationId)?.title || '')
            }}
          >
          ...
          <button
            ...
            className="conversation-context-menu__item"
            onClick={() => {
              setProjectPicker({ conversationId: contextMenu.conversationId })
              setContextMenu(null)
            }}
          >
            <FolderTree size={14} strokeWidth={2} aria-hidden="true" />
            {t('nav.moveToProject')}
          </button>
          <div className="conversation-context-menu__separator" role="separator" />
          <button
            type="button"
            role="menuitem"
            className="conversation-context-menu__item conversation-context-menu__item--danger"
            onClick={() => {
              handleArchive(contextMenu.conversationId)
              setContextMenu(null)
            }}
          >
