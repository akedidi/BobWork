...
function PluginListRow({
  ...
  return (
    <div className={`skill-list-row ${selected ? 'selected' : ''} ${enabled ? '' : 'disabled'}`}>
      ...
      <label className="skill-switch" title={enabled ? 'Désactiver' : 'Activer'}>
        <input type="checkbox" checked={enabled} disabled={toggling} aria-label={`${enabled ? 'Désactiver' : 'Activer'} le plugin ${plugin.name}`} onChange={event => onToggle(event.target.checked)} />
...
export default function PluginsView() {
  ...
  const { filter, setFilter, search, setSearch, visiblePlugins, visibleSections } = usePluginFilter(plugins)
          ...
          <section className="skills-browser">
            <div className="skills-toolbar">
              ...
              <div className="skills-filter-pills">
                {([
                  ['all', 'plugins.filterAll'],
                  ['business', 'plugins.filterBusiness'],
                  ['ibm', 'plugins.filterIbm'],
                  ['enabled', 'plugins.filterEnabled'],
                  ['disabled', 'plugins.filterDisabled'],
            ...
            <div className="skills-list">
              {loadError ? null : loading ? <Empty text="Chargement…" /> : visiblePlugins.length === 0 ? <Empty text={plugins.length ? t('plugins.noneFound') : t('plugins.empty')} /> : visibleSections.map(section => (
                <div className="skills-list-section" key={section.id} data-testid={`plugin-section-${section.id}`}>
                  <h3 className="skills-list-section-title">{t(section.titleKey)}</h3>
                  {section.plugins.map(plugin => (
                    <PluginListRow
                      ...
                      onDelete={() => void removePlugin(plugin)}
                      onToggle={enable => void changeEnabled(plugin, enable)}
                    />
                  ))}
                </div>
              ))}
