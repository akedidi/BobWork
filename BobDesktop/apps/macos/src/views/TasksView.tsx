...
import { normalizeAssistantMarkdown } from '../lib/chatUtils'
  ...
  return (
    <aside className="task-drawer">
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14 }}>
        <strong className="task-drawer-title">{t('tasks.detail')}</strong>
      ...
      {loading || !detail ? <Empty icon={<Spinner />} text={t('common.loading')} /> : <>
        ...
        {detail.task.summary && <Section title={t('tasks.result')}><TaskResultMarkdown value={detail.task.summary} /></Section>}
...
function TaskResultMarkdown({ value }: { value: string }) {
  return <div className="task-detail-markdown prose">
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      components={{
        table: ({ node: _node, ...props }) => (
          <div className="markdown-table-scroll" role="region" aria-label="Tableau du résultat" tabIndex={0}>
            <table {...props} />
          </div>
        ),
      }}
    >
      {normalizeAssistantMarkdown(value)}
    </ReactMarkdown>
  </div>
}
