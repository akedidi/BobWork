...
describe('TasksView pinning', () => {
  ...
  it('renders a task result as GFM Markdown instead of escaped plain text', async () => {
    const markdownTask = {
      ...task,
      summary: 'Fichier écrit : `bell_results_3d.html`\n\n## Rendu 3D\n\n| Élément | Description |\n| --- | --- |\n| **Barres** | Animation fluide |',
    }
    mocks.getTasks.mockResolvedValue([markdownTask])
    mocks.getTaskDetail.mockResolvedValue({ task: markdownTask, runs: [], inputs: [], outputs: [], events: [] })
    render(<MemoryRouter><TasksView /></MemoryRouter>)

    fireEvent.click(await screen.findByText('Préparer le rapport client'))
    expect(await screen.findByRole('heading', { name: 'Rendu 3D' })).toBeVisible()
    expect(screen.getByRole('table')).toBeVisible()
    expect(screen.getByText('bell_results_3d.html')).toHaveProperty('tagName', 'CODE')
  })
})
