...
import {
  createProject, deleteProject, getBobModes, getConversations,
  getProject, getProjects, updateProject,
} from '../lib/ipc'
import type { BobMode, Conversation, Project } from '@bob-work/shared-types'
...
export default function ProjectView() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setLoadError(null)
    const base = getBobModes().then(nextModes => { setModes(nextModes) })
    if (isNew || !id) {
    ...
    try {
      if (isNew) {
        let created = await createProject({
          ...
          language: 'auto', defaultMode: form.defaultMode,
        })
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
    ...
    <div className="project-content">
      {(isNew || editing) ? <ProjectEditor form={form} setForm={setForm} modes={modes} saving={saving} onSave={save} /> : project && <>
...
function ProjectEditor({ form, setForm, modes, saving, onSave }: {
  form: Form; setForm: React.Dispatch<React.SetStateAction<Form>>; modes: BobMode[]; saving: boolean; onSave: () => void
}) {
  const t = useT()
  ...
  return <div className="project-editor">
    ...
    <label>{t('project.name')}<input autoFocus value={form.name} onChange={event => set('name', event.target.value)} /></label>
...
function fromProject(project: Project): Form {
  return {
    name: project.name,
    ...
    language: 'auto',
    defaultMode: project.defaultMode ?? 'agent',
    memoryEnabled: project.memoryEnabled,
    allowedFiles: project.allowedFiles,
    allowedPlugins: [],
    allowedIntegrations: [],
  }
}
function updateInput(form: Form) {
  return {
    name: form.name.trim(),
    ...
    allowedFiles: form.allowedFiles,
    allowedPlugins: [],
    allowedIntegrations: [],
  }
}
