...
describe('PluginBuilderView', () => {
  ...
  it('parcourt le wizard et lance la génération avec un brief structuré', () => {
    ...
    expect(screen.getByText('CLI locale')).toBeVisible()
    expect(screen.getByText('Script shell')).toBeVisible()
    expect(screen.getByText('Binaire embarqué')).toBeVisible()
    fireEvent.click(screen.getByText('Binaire embarqué'))
    fireEvent.click(screen.getByRole('button', { name: 'Continuer' }))
    ...
    expect(useAppStore.getState().builderSession?.brief).toContain('Binaire embarqué')
    expect(useAppStore.getState().builderSession?.brief).toContain('Commandes locales (avec accord)')
