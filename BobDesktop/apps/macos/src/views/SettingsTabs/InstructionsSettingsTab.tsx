          ...
          <Card>
            <SettingsFields settings={settings} error={settingsError} loadingLabel={loadingLabel}>
              {s => <>
                ...
                <ToggleRow
                  title={t('settings.autoCondenseEnabled')}
                  description={t('settings.autoCondenseEnabledDesc')}
                  value={s.autoCondenseEnabled}
                  onChange={value => change('autoCondenseEnabled', value)}
                />
                <NumberRow
                  title={t('settings.autoCondenseThreshold')}
                  description={t('settings.autoCondenseThresholdDesc')}
                  value={Math.round(s.autoCondenseThreshold * 100)}
                  min={50}
                  max={99}
                  onChange={value => change('autoCondenseThreshold', Math.min(99, Math.max(50, value)) / 100)}
                />
              </>}
            </SettingsFields>
          </Card>
