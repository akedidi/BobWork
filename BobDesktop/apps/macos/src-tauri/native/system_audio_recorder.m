...
// Do not use Clang's @available lowering here. This Objective-C object is linked
// by rustc with -nodefaultlibs, so a completely clean Release build does not
// automatically receive Clang's ___isPlatformVersionAtLeast helper. Foundation's
// runtime version check has the same behavior without that hidden linker input.
static BOOL BWSystemAudioIsAvailable(void) {
    NSOperatingSystemVersion required = { .majorVersion = 15, .minorVersion = 0, .patchVersion = 0 };
