---
name: Agent Review
description: "Find actionable bugs in code changes with a read-only, defect-first review of a diff, commit, or branch."
icon: codegraph
user-invocable: true
---
...
- Review uncommitted changes, a commit, a pull request diff, or a base-branch comparison
- Another agent or the user asks for a defect-first code review before merge
- Custom review instructions scoped to a specific path or change set

## Review the change

1. Read the applicable `AGENTS.md` instructions when present in the repository.
...
- Write the **entire review output** in the same language as the user's prompt (the message that invoked this skill), including finding titles, assessment headings, and summary text.
- Keep file paths, line numbers, code identifiers, and priority tags (`P0`–`P3`) unchanged.
- When the prompt language is ambiguous, use the language of the prompt's main request sentence.
- Never mix languages in a single review unless the user explicitly requests bilingual output.

## Forbidden

- Editing source files, tests, or configuration as part of the review
- Creating commits, branches, or pull-request comments
- Launching another agent or subagent to perform the review
- Reporting issues that are not introduced by the reviewed change
