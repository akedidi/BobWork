...
app_command, capture_screen, desktop_click, desktop_type, desktop_scroll, press_key,
...
TOOLS = [
    ...
    },
    {
        "name": "desktop_scroll",
        "description": (
            "Fait défiler la zone observée aux coordonnées écran. Une observation "
            "get_app_state ou capture_screen est requise avant l’action."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "x": {"type": "number"},
                "y": {"type": "number"},
                "delta_y": {"type": "integer", "minimum": -1200, "maximum": 1200},
                "delta_x": {"type": "integer", "minimum": -1200, "maximum": 1200},
            },
...
def _begin_guarded_action(name: str) -> dict | None:
    global ACTION_COUNT, OBSERVED_SINCE_ACTION
    if ACTION_COUNT >= MAX_ACTIONS:
        return {"ok": False, "error": "Limite de 20 actions atteinte. Arrête la boucle et rends le contrôle à l’utilisateur."}
    if not OBSERVED_SINCE_ACTION:
        return {
            "ok": False,
            "error": (
                f"Observation requise avant {name}. Appelle get_app_state ou capture_screen, "
                "puis décide d’une seule action à partir de cet état."
            ),
        }
    ACTION_COUNT += 1
    OBSERVED_SINCE_ACTION = False
...
def _action_risk(name: str, arguments: dict) -> str:
    detail = " ".join(str(arguments.get(key) or "") for key in ("label", "commands", "key")).lower()
    sensitive = (
        "delete", "remove", "erase", "send", "submit", "publish", "purchase", "buy", "pay",
        "supprimer", "effacer", "envoyer", "publier", "acheter", "payer",
        "borrar", "eliminar", "enviar", "publicar", "comprar", "pagar",
    )
    if name == "app_command" or any(word in detail for word in sensitive):
        return "high"
    if name in {"ui_set_value", "desktop_type", "press_key"}:
        return "medium"
    return "low"


def _authorize_action(name: str, arguments: dict) -> dict | None:
    if e2e_mode():
        return None
    app = str(arguments.get("app") or ACTIVE_TARGET_APP or "application")
    label = str(arguments.get("label") or "").strip()
    descriptions = {
        "ui_click": f"Bob souhaite cliquer sur « {label or 'un élément'} » dans {app}.",
        "ui_set_value": f"Bob souhaite modifier le champ « {label or 'texte'} » dans {app}.",
        "app_command": f"Bob souhaite exécuter une commande de l’application {app}.",
        "desktop_click": f"Bob souhaite cliquer dans {app}.",
        "desktop_type": f"Bob souhaite saisir du texte dans {app}.",
        "desktop_scroll": f"Bob souhaite faire défiler {app}.",
        "press_key": f"Bob souhaite envoyer une touche à {app}.",
    }
    result = _run_native_via_bob_work(
        "authorize",
        operation=name,
        app_name=app,
        summary=descriptions.get(name, f"Bob souhaite agir dans {app}."),
        risk_level=_action_risk(name, arguments),
        task_id=os.environ.get("BOB_WORK_TASK_ID", ""),
    )
    if result.returncode != 0:
        return {"ok": False, "error": result.stderr or "Action refusée par Bob Work."}
    return None


def _mark_observed() -> None:
    global OBSERVED_SINCE_ACTION
    OBSERVED_SINCE_ACTION = True
...
def desktop_scroll(x: float, y: float, delta_y: int, delta_x: int = 0) -> dict:
    if e2e_mode():
        return {"ok": True, "mode": "e2e", "x": x, "y": y, "delta_y": delta_y, "delta_x": delta_x}
    require_macos()
    result = _run_native_via_bob_work(
        "scroll", x=float(x), y=float(y), delta_y=int(delta_y), delta_x=int(delta_x)
    )
    if result.returncode != 0:
        return {"ok": False, "x": x, "y": y, **accessibility_hint(result.stderr)}
    return {"ok": True, "x": int(x), "y": int(y), "delta_y": int(delta_y), "delta_x": int(delta_x)}
...
def handle_call(name: str, arguments: dict) -> dict:
    guarded = {"ui_click", "ui_set_value", "app_command", "desktop_click", "desktop_type", "desktop_scroll", "press_key"}
    if name in guarded:
        guard_error = _begin_guarded_action(name)
        if guard_error:
            return tool_result(guard_error)
        approval_error = _authorize_action(name, arguments)
        if approval_error:
            return tool_result(approval_error)
    result = _handle_call(name, arguments)
    if name in {"get_app_state", "capture_screen"}:
    ...
    if name == "desktop_scroll":
        return tool_result(
            desktop_scroll(
                float(arguments.get("x") or 0),
                float(arguments.get("y") or 0),
                int(arguments.get("delta_y") or 0),
                int(arguments.get("delta_x") or 0),
            )
        )
    if name == "press_key":
        return tool_result(
