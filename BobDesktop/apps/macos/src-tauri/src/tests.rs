...
mod tests {
    mod database_tests {
        ...
        #[test]
        fn migrated_schema_has_db_connections_table() {
            let db = Database::new_in_memory().expect("database");
            db.run_migrations().expect("migrations");
            let conn = db.conn.lock().unwrap();
            let count: i64 = conn
    ...
    mod conversation_tests {
        ...
        fn test_add_and_get_messages() {
            ...
            assert_eq!(msgs[1].file_changes[0]["changeType"], "modified");
        }

        #[test]
        fn latest_conversation_modification_moves_it_to_top() {
            let db = temp_db();
            let svc = ConversationService::new();
            ...
            svc.add_message(
                ...
                AddMessageInput {
                    ...
                    content: "Nouvelle réponse de Bob".into(),
                    attachments: None,
                    sources: None,
                },
            )
            ...
            svc.update_title(&db, &second.id, "Deuxième renommée")
                .expect("rename");
            assert_eq!(svc.get_all(&db, None).unwrap()[0].id, second.id);
        }
