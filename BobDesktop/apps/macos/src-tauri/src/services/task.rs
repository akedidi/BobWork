...
impl TaskService {
    ...
    pub fn latest_resumable_shell_task_id(
        &self,
        db: &Database,
        conversation_id: &str,
    ) -> AppResult<Option<String>> {
        let tasks = self.list_for_conversation(db, conversation_id)?;
        Ok(tasks.into_iter().rev().find_map(|task| {
            if task.resumable {
                task.shell_task_id.filter(|id| !id.trim().is_empty())
            } else {
                None
            }
        }))
