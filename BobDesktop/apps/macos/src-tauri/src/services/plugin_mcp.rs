    ...
    fn prepare(
        ...
        for (id, value) in servers {
            ...
            let (transport, config) = if let Some(url) = config.get("url").and_then(Value::as_str) {
                ...
                let command = resolve_local_runtime_command(bundle_dir, command)?;
                config.insert(
                    "command".into(),
...
fn resolve_local_runtime_command(
    bundle_dir: &Path,
    command: &str,
    ...
    if path.is_absolute() || command.contains('/') || command.contains('\\') {
        ...
        if !candidate.starts_with(root) || !candidate.is_file() {
            return Err(AppError::Security(
                "Local MCP executable must belong to its plugin bundle".into(),
            ));
        }
        return Ok(candidate);
    }
    if !matches!(command, "python3" | "node" | "bash" | "sh" | "zsh") {
        return Err(AppError::Security(format!(
            "Local MCP command {command} must be a declared private dependency"
        )));
    }
    which::which(command).map_err(|_| {
        AppError::NotFound(format!("Approved local runtime {command} is not installed"))
    })
}
