...
impl ConversationService {
    ...
    pub fn rewind_conversation_from_message(
        ...
        if title_reset {
            conn.execute(
                "UPDATE conversations SET title = 'Nouvelle conversation', bob_context_state = '{}', summary = NULL, date = ?1 WHERE id = ?2",
                params![modified_at, conversation_id],
            )?;
        } else {
            conn.execute(
                "UPDATE conversations SET bob_context_state = '{}', summary = NULL, date = ?1 WHERE id = ?2",
                params![modified_at, conversation_id],
            )?;
        }
