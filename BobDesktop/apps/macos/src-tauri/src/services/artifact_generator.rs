...
impl ArtifactGeneratorService {
    ...
    /// Runtime Manager-resolved Shared Python executable when supplied.
    pub fn generate_with_python(
        ...
        let (file_path, validation_status, validation_notes) = match input.artifact_type.as_str() {
            ...
            "pdf" => self.generate_pdf(&id, &input.title, &input.content, artifacts_dir, python)?,
            "markdown" | "text" | "html" => {
                self.generate_text(&id, &input.artifact_type, &input.content, artifacts_dir)?
            }
            "sketch" => self.generate_sketch(&id, &input.content, artifacts_dir)?,
            _ => {
        ...
        drop(conn);
        crate::services::artifact_runtime::ArtifactRuntime::new().attach(
            db,
            &crate::services::artifact_runtime::ArtifactRuntimeMetadata {
                artifact_id: id.clone(),
                artifact_kind: input.artifact_type.clone(),
                owner_plugin_id: None,
                producer_runtime_id: None,
                project_id: None,
                conversation_id: input.conversation_id.clone(),
                workspace_reference: Some(file_path_str.clone()),
                metadata: serde_json::json!({"generator":"bob-work"}),
...
"#,
    ...
    fn generate_pdf(
        ...
        let pdf_result = std::process::Command::new(python.unwrap_or_else(|| Path::new("python3")))
            .arg("-c")
            .arg(format!(
    ...
    fn generate_sketch(
        ...
        let path = dir.join(format!("{id}.sketch"));
        let document: serde_json::Value = serde_json::from_str(content)
            .map_err(|error| AppError::ValidationFailed(error.to_string()))?;
        std::fs::write(&path, crate::services::designer::render_sketch(&document)?)?;
        Ok((
            path,
            "valid".to_string(),
            Some(
                "Structured Sketch interchange package; third-party import fidelity can vary."
                    .into(),
            ),
        ))
    }
