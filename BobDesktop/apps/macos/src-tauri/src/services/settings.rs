...
impl SettingsService {
    ...
    pub fn get(&self, db: &Database) -> AppResult<AppSettings> {
        ...
        Ok(AppSettings {
            ...
            cross_conversation_context: get_val("cross_conversation_context")
                .map(|v| v.as_str() == "true")
                .unwrap_or(false),
            persistent_memory_enabled: get_val("persistent_memory_enabled")
                .map(|v| v.as_str() == "true")
                .unwrap_or(false),
            persistent_memory_engine: get_val("persistent_memory_engine")
                .and_then(|v| serde_json::from_str(v).ok())
                .unwrap_or_else(|| "local".to_string()),
            remote_control_enabled: get_val("remote_control_enabled")
                .map(|v| v.as_str() == "true")
                .unwrap_or(false),
            mcp_gateway_enabled: get_val("mcp_gateway_enabled")
                .map(|v| v.as_str() == "true")
                .unwrap_or(false),
            mcp_gateway_tools: get_val("mcp_gateway_tools")
                .and_then(|v| serde_json::from_str(v).ok())
                .unwrap_or_default(),
            auto_condense_enabled: get_val("auto_condense_enabled")
                .map(|v| v.as_str() != "false")
                .unwrap_or(true),
            auto_condense_threshold: get_val("auto_condense_threshold")
                .and_then(|v| v.parse().ok())
                .filter(|value: &f64| value.is_finite() && *value > 0.0 && *value < 1.0)
                .unwrap_or(crate::services::bob_context::DEFAULT_AUTO_CONDENSE_THRESHOLD),
            location_enabled: get_val("location_enabled")
                .map(|v| v.as_str() == "true")
                .unwrap_or(false),
            current_latitude: get_val("current_latitude")
                .and_then(|v| serde_json::from_str(v).ok()),
            current_longitude: get_val("current_longitude")
                .and_then(|v| serde_json::from_str(v).ok()),
            current_location_updated_at: get_val("current_location_updated_at")
                .and_then(|v| serde_json::from_str(v).ok()),
        })
    }
    ...
    pub fn update_all(&self, db: &Database, settings: &AppSettings) -> AppResult<()> {
        let now = Utc::now().to_rfc3339();
        let conn = db.conn.lock().unwrap();
        let default_mode = if settings.default_mode == "general_work" {
            "agent"
        } else {
        ...
        let updates = vec![
            ...
            ("default_mode", serde_json::to_string(default_mode).unwrap()),
            ...
            (
                ...
                settings.persistent_memory_enabled.to_string(),
            ),
            (
                "persistent_memory_engine",
                serde_json::to_string(&settings.persistent_memory_engine).unwrap(),
            ),
            (
            ...
            (
                "mcp_gateway_enabled",
                settings.mcp_gateway_enabled.to_string(),
            ),
            (
                "mcp_gateway_tools",
                serde_json::to_string(&settings.mcp_gateway_tools).unwrap(),
            ),
            (
                "auto_condense_enabled",
                settings.auto_condense_enabled.to_string(),
            ),
            (
                "auto_condense_threshold",
                settings.auto_condense_threshold.to_string(),
            ),
            ("location_enabled", settings.location_enabled.to_string()),
            (
                "current_latitude",
                serde_json::to_string(&settings.current_latitude).unwrap(),
            ),
            (
                "current_longitude",
                serde_json::to_string(&settings.current_longitude).unwrap(),
            ),
            (
                "current_location_updated_at",
                serde_json::to_string(&settings.current_location_updated_at).unwrap(),
            ),
        ];
