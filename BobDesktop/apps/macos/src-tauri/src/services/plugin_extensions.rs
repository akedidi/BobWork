    ...
    pub fn resource_status(
        ...
        for (index, resource) in resources.iter().enumerate() {
            ...
            let (state, message, setup_hint) = match kind.as_str() {
                ...
                "api-key" => {
                    ...
                    if api_key_available(&key) {
                        ...
                        (
                            "needs_key".into(),
                            format!("Définissez {key} pour activer cette source."),
                            Some(format!(
                ...
                "mcp" => {
                    if let Some(server_id) = mcp_server.as_deref() {
                        ...
                        if api_key_available(key) {
                            ...
                            (
                                "inactive".into(),
                                "Aucun MCP distant configuré.".into(),
                                Some(format!(
                ...
                "stdio-cli" | "bundled-bin" | "shell" | "node-cli" => {
                    let plugin_name = manifest
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or(plugin_id);
                    crate::services::plugin_local_runtime::resource_readiness(
                        plugin_name,
                        manifest,
                        resource,
                        &kind,
                ...
                "web-reference" => {
                    if resource_https_url(resource).is_some() {
                        (
                            "ready".into(),
                            format!(
                                "Référence officielle embarquée — Bob peut la consulter{} (aucune connexion à configurer).",
                                if optional { " si le sujet l’exige" } else { "" }
                            ),
                            None,
                        )
                    } else {
                        (
                            "needs_setup".into(),
                            "Référence web sans URL https valide.".into(),
                            None,
                        )
                    }
                }
                "database" => {
                    let connection_id = resource
                        ...
                        .and_then(Value::as_str)
                        .or_else(|| resource.get("provider").and_then(Value::as_str));
                    match connection_id.and_then(|id| {
                        crate::services::db_connection::DbConnectionService::new()
                            .get_by_id(db, id)
                            .ok()
                            .flatten()
                    }) {
                        Some(connection) if connection.enabled => (
                            "ready".into(),
                            format!("Connexion DB « {} » prête.", connection.name),
                            None,
                        ),
                        Some(_) => (
                            "inactive".into(),
                            "Connexion DB désactivée.".into(),
                            Some("Activez-la dans Intégrations → DB.".into()),
                        ),
                        None => (
                            "needs_setup".into(),
                            "Aucune connexion DB liée.".into(),
                            Some("Liez une connexion dans le détail du plugin ou créez-en une dans Intégrations → DB.".into()),
                        ),
                    }
                }
                "pdf" | "spreadsheet" | "file" => {
                    ("ready".into(), "Fichier ressource du plugin.".into(), None)
                }
                other => (
                    "inactive".into(),
            ...
            let configure_url = match (kind.as_str(), env_key.as_deref()) {
                ("web-reference", _) => resource_https_url(resource),
        ...
        if let Ok(files) = overlay.list_files(plugin_id) {
            for file in files {
                out.push(PluginResourceStatus {
                    ...
                    optional: true,
                    state: "ready".into(),
                    message: "Fichier uploadé pour ce plugin.".into(),
                    setup_hint: None,
                    configure_tab: None,
                    env_key: None,
                    configure_url: Some(file.path),
                });
            }
        }
        if let Ok(linked) = overlay.list_linked_databases(plugin_id) {
            for link in linked {
                let connection = crate::services::db_connection::DbConnectionService::new()
                    .get_by_id(db, &link.connection_id)
                    .ok()
                    .flatten();
                let (state, message, setup_hint): (&str, String, Option<String>) =
                    match connection.as_ref() {
                        Some(item) if item.enabled => (
                            "ready",
                            format!("Connexion DB « {} » liée.", item.name),
                            None,
                        ),
                        Some(_) => (
                            "inactive",
                            "Connexion DB liée mais désactivée.".into(),
                            Some("Activez-la dans Intégrations → DB.".into()),
                        ),
                        None => (
                            "needs_setup",
                            "Connexion DB liée introuvable.".into(),
                            Some("Recréez la connexion dans Intégrations → DB.".into()),
                        ),
                    };
                out.push(PluginResourceStatus {
                    id: format!("database-{}", link.connection_id),
                    label: connection
                        .as_ref()
...
fn resource_https_url(resource: &Value) -> Option<String> {
    let raw = resource.get("url").and_then(Value::as_str)?.trim();
    if raw.len() > 2048 {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn web_reference_sources_are_ready_documentation_not_inactive_connectors() {
        let db = temp_db();
        let (_root, bob) = temp_bob();
        let manifest = serde_json::json!({
            "resources": [
        ...
        let status = PluginExtensionService::new()
            .resource_status("builtin-ibm-agentic-designer", &manifest, &db, &bob)
            .expect("status");
        assert_eq!(status[0].state, "ready");
        assert!(status[0]
            .configure_url
            .as_deref()
            .is_some_and(|url| url.starts_with("https://www.ibm.com/garage")));
        assert!(status[0].message.contains("Référence officielle"));
        assert_eq!(status[1].state, "needs_setup");
        assert!(status[1].configure_url.is_none());
    }
}
