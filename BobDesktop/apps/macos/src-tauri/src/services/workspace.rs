...
impl WorkspaceService {
    ...
    fn list_mcp_servers_from_home(home: &std::path::Path) -> Vec<McpServer> {
        ...
        for path in candidates {
            ...
            for (name, raw) in servers {
                ...
                let builtin = mcp_server_is_builtin(home, name, raw);
                merged.insert(
                    name.clone(),
                    McpServer {
                        name: name.clone(),
        ...
        // User-created connectors remain immediately visible above the longer
        // catalog of MCP servers managed by Bob Work and built-in plugins.
        values.sort_by(|a, b| a.builtin.cmp(&b.builtin).then_with(|| a.name.cmp(&b.name)));
        values
    }
    ...
    pub fn save_mcp_server(&self, bob_path: &str, input: SaveMcpServerInput) -> AppResult<()> {
        ...
        if let Some(original_name) = input.original_name.as_deref() {
            validate_slug(original_name)?;
        }
        self.ensure_mcp_server_mutable(input.original_name.as_deref().unwrap_or(&input.name))?;
        if input.original_name.as_deref() != Some(input.name.as_str()) {
            self.ensure_mcp_server_mutable(&input.name)?;
        }
        ...
        let previous = input
            .original_name
            .as_deref()
            .and_then(|name| self.read_mcp_server_config(name));
        ...
        if remote {
            ...
            let edited_url = preserve_redacted_url_secrets(
                input.command_or_url.trim(),
                previous
                    .as_ref()
                    .and_then(|value| value.get("url"))
                    .and_then(Value::as_str),
            );
            config.insert("url".into(), Value::String(edited_url));
        } else {
            config.insert(
                "command".into(),
        ...
        if input.headers.is_some() || input.original_name.is_some() {
            if !remote {
        ...
        if let Some(original_name) = input.original_name.as_deref() {
            if original_name != input.name {
                self.delete_mcp_server(bob_path, original_name)?;
            }
        }
        Ok(())
    }
    ...
    fn ensure_mcp_server_mutable(&self, name: &str) -> AppResult<()> {
        let home = dirs::home_dir()
            .ok_or_else(|| AppError::Io("Dossier utilisateur introuvable".into()))?;
        if mcp_server_is_builtin_by_name(&home, name) {
            return Err(AppError::ValidationFailed(format!(
                "Le connecteur MCP {name} est intégré et doit être géré depuis son plugin Bob Work"
            )));
        }
        Ok(())
    }
    ...
    pub fn publish_usage_status<R: tauri::Runtime>(
        ...
        let status = Self::new().usage_status_with_refresh(&db, force);
...
pub(crate) fn snapshot_to_status(
    ...
    let keyed = crate::services::bob_usage::api_key_configured();
    UsageStatus {
        available,
        ...
        account_email: keyed.then_some(data.account_email).flatten(),
        instance_name: keyed.then_some(data.instance_name).flatten(),
        message,
    }
}

/// Bob Work reserves the `bob-work-*` namespace for native connectors.
/// Plugin MCP names use `bw-*`, so their ownership is resolved from the
/// deployed bundle: MCPs from user-authored plugins remain personal.
fn mcp_server_is_builtin(home: &Path, name: &str, raw: &Value) -> bool {
    if name.starts_with("bob-work-") {
        return true;
    }

    let Some(cwd) = raw.get("cwd").and_then(Value::as_str) else {
        return false;
    };
    let skills_root = home.join(".bob/skills");
    let Ok(relative) = Path::new(cwd).strip_prefix(&skills_root) else {
        return false;
    };
    let Some(bundle_name) = relative.components().next() else {
        return false;
    };
    let bundle_dir = skills_root.join(bundle_name.as_os_str());

    if std::fs::read_to_string(bundle_dir.join(".bob-work-plugin-id"))
        .ok()
        .is_some_and(|owner| owner.trim().starts_with("builtin-"))
    {
        return true;
    }

    std::fs::read_to_string(bundle_dir.join(".bob-work-plugin.json"))
        .ok()
...
fn mcp_server_is_builtin_by_name(home: &Path, name: &str) -> bool {
    if name.starts_with("bob-work-") {
        return true;
    }
    for path in [
        ...
        if mcp_server_is_builtin(home, name, &raw) {
            return true;
        }
    }
    false
...
mod mcp_tests {
    ...
    fn parses_and_redacts_native_bob_mcp_configuration() {
        ...
        assert_eq!(servers[0].raw["env"]["TOKEN"], "<redacted>");
    ...
    #[test]
    fn marks_builtin_mcp_owners_and_lists_personal_connectors_first() {
        let home = isolated_home();
        let settings = home.join(".bob/settings");
        let builtin_bundle = home.join(".bob/skills/maps");
        let personal_bundle = home.join(".bob/skills/my-tools");
        std::fs::create_dir_all(&settings).unwrap();
        std::fs::create_dir_all(&builtin_bundle).unwrap();
        std::fs::create_dir_all(&personal_bundle).unwrap();
        std::fs::write(
            builtin_bundle.join(".bob-work-plugin-id"),
            "builtin-map-tools",
        )
        .unwrap();
        std::fs::write(
            personal_bundle.join(".bob-work-plugin-id"),
            "agentic-my-tools",
        )
        .unwrap();
        std::fs::write(
            ...
            serde_json::json!({
                "mcpServers": {
                    "bob-work-computer-use": {"command": "python3"},
                    "bw-map-tools-routing": {"command": "python3", "cwd": builtin_bundle},
                    "bw-my-tools-helper": {"command": "python3", "cwd": personal_bundle},
                    "airline-operations": {"url": "https://example.test/mcp"}
                }
            })
            .to_string(),
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                "bw-map-tools-routing",
            ]
        );
        assert!(!servers[0].builtin);
        assert!(!servers[1].builtin);
        assert!(servers[2].builtin);
        assert!(servers[3].builtin);
        assert!(super::mcp_server_is_builtin_by_name(
            &home,
            "bob-work-computer-use"
        ));
    ...
    fn editing_a_redacted_api_url_preserves_its_secret_query_value() {
        let edited = super::preserve_redacted_url_secrets(
            ...
            Some("https://api.example.test/v1?api_key=real-secret&format=xml"),
        );
        assert!(edited.contains("/v2"));
        assert!(edited.contains("api_key=real-secret"));
    ...
    if let Some(map) = value.as_object_mut() {
        for key in ["env", "headers", "token", "authorization"] {
            if let Some(existing) = map.get(key).cloned() {
                // Retain only field names so the editor can identify the auth
                // shape without ever receiving the stored values.
                let redacted = existing
                    .as_object()
                    .map(|values| {
                        values
                            .keys()
                    ...
                    .unwrap_or_else(|| {
                        ...
                        marker.insert("_redacted".into(), Value::Bool(true));
                        marker
                    });
...
fn preserve_redacted_url_secrets(edited: &str, previous: Option<&str>) -> String {
    let Some(previous) = previous else {
        return edited.to_string();
    };
    let (Ok(mut edited_url), Ok(previous_url)) =
        (url::Url::parse(edited), url::Url::parse(previous))
    else {
        return edited.to_string();
    };
    let sensitive = ["api_key", "apikey", "access_token", "token", "key", "auth"];
    let old_pairs: std::collections::HashMap<String, String> = previous_url
        .query_pairs()
        .filter(|(key, _)| sensitive.iter().any(|item| key.eq_ignore_ascii_case(item)))
        .map(|(key, value)| (key.into_owned(), value.into_owned()))
        .collect();
    if old_pairs.is_empty() {
        return edited.to_string();
    }
    let pairs: Vec<(String, String)> = edited_url
        .query_pairs()
        .map(|(key, value)| {
    ...
    if !query.is_empty() {
        edited_url.set_query(Some(&query));
    }
    edited_url.to_string()
}
