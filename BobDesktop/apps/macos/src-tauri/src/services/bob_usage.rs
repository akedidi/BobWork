...
/// Other Bob apps share the same IBM budget — pull a fresh snapshot at least this often.
pub const USAGE_REFRESH_INTERVAL_SECS: u64 = 60;
...
impl BobUsageService {
    ...
    pub fn refresh_snapshot(&self, db: &Database) -> AppResult<Option<UsageSnapshotData>> {
        ...
        let snapshot = UsageSnapshotData {
            ...
            account_email: profile_account_email(&profile.raw),
            instance_name: profile_instance_name(&profile.raw),
            ...
            Ok((used, remaining, unit, raw, captured_at)) => {
                ...
                Ok(Some(UsageSnapshotData {
                    ...
                    account_email: meta
                        ...
                        .and_then(Value::as_str)
                        .map(str::to_string)
                        .or_else(|| profile_account_email(meta.get("profile").unwrap_or(&meta))),
                    instance_name: meta
                        .get("instanceName")
                        .and_then(Value::as_str)
                        .map(str::to_string)
                        .or_else(|| profile_instance_name(meta.get("profile").unwrap_or(&meta))),
                    captured_at,
                }))
            }
            ...
            Some(snapshot) => DateTime::parse_from_rfc3339(&snapshot.captured_at)
                .map(|value| {
                    (Utc::now() - value.with_timezone(&Utc)).num_seconds()
                        >= USAGE_REFRESH_INTERVAL_SECS as i64
                })
                .unwrap_or(true),
        }
    }
...
/// True when an IBM Bob inference API key is in the vault or environment.
pub fn api_key_configured() -> bool {
    read_vault_or_env_api_key().is_some()
}
...
pub(crate) fn profile_account_email(raw: &Value) -> Option<String> {
    [
        raw.get("email"),
        raw.pointer("/user/email"),
        raw.get("user_id"),
        raw.pointer("/user/user_id"),
    ]
    .into_iter()
    .find_map(|value| {
        value
...
pub(crate) fn profile_instance_name(raw: &Value) -> Option<String> {
    raw.pointer("/instances/0/instance_name")
        .and_then(Value::as_str)
        .map(str::trim)
...
mod tests {
    ...
    fn parses_profile_rows_with_team_budget() {
        ...
        assert_eq!(profile_account_email(&raw), None);
        assert_eq!(profile_instance_name(&raw).as_deref(), Some("IBM Internal"));
    }

    #[test]
    fn extracts_account_email_and_instance_from_ibm_profile() {
        let raw = serde_json::json!({
            "user_id": "akedidi@fr.ibm.com",
            "instances": [{ "instance_name": "IBM Internal", "teams": [{ "name": "default" }] }]
        });
        assert_eq!(
            profile_account_email(&raw).as_deref(),
            Some("akedidi@fr.ibm.com")
        );
        assert_eq!(profile_instance_name(&raw).as_deref(), Some("IBM Internal"));
    }
    ...
    fn snapshot_at(captured_at: String) -> UsageSnapshotData {
        UsageSnapshotData {
            used_amount: Some(10.0),
            remaining_amount: Some(150.0),
    ...
    #[test]
    fn refreshes_when_no_snapshot_exists() {
        assert!(BobUsageService::should_refresh(None));
    }

    #[test]
    fn refreshes_when_snapshot_is_at_least_one_minute_old() {
        let stale = snapshot_at((Utc::now() - chrono::Duration::seconds(90)).to_rfc3339());
        assert!(BobUsageService::should_refresh(Some(&stale)));
    }

    #[test]
    fn keeps_a_snapshot_captured_less_than_a_minute_ago() {
        let fresh = snapshot_at((Utc::now() - chrono::Duration::seconds(15)).to_rfc3339());
        assert!(!BobUsageService::should_refresh(Some(&fresh)));
    }

    #[test]
    fn refreshes_when_captured_at_is_invalid() {
        assert!(BobUsageService::should_refresh(Some(&snapshot_at(
            "not-a-date".into()
        ))));
    }
}
