...
impl ArtifactService {
    ...
    /// Complete artifact catalogue used by the authenticated remote-control API.
    /// Pagination is applied by that API after its origin/search filters so the
    /// mobile gallery does not silently lose older files.
    pub fn get_all_for_remote(&self, db: &Database) -> AppResult<Vec<Artifact>> {
        let conn = db.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT id, type, title, file_path, version, preview_path,
             origin, sources, validation_status, validation_notes, exported, created_at, size
    ...
    pub fn register_external(
        ...
        crate::services::artifact_runtime::ArtifactRuntime::new().attach(
            ...
            &crate::services::artifact_runtime::ArtifactRuntimeMetadata {
                ...
                artifact_kind: artifact_type.clone(),
                owner_plugin_id: None,
                producer_runtime_id: None,
                project_id: None,
                conversation_id: conversation_id.map(str::to_string),
                workspace_reference: Some(path_str.clone()),
                metadata: serde_json::json!({"origin":"bob-shell"}),
